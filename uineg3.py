...
class FloorPlanGUI:
    def __init__(self, root):
        ...
        ttk.Button(self.nav_frame, text="Generate Blocks",
                   command=self.generate_and_show_blocks).pack(side=tk.RIGHT, padx=5)

        # Block visualization controls
        self.show_blocks_var = tk.BooleanVar(value=False)
    ...
    def generate_and_show_blocks(self):
        """Generate blocks and update visualization"""
        if not self.floor_plan:
            messagebox.showwarning("Warning", "Please generate a floor plan first")
            return

        blocks, residuals = self.floor_plan.generate_blocks()

        # Update statistics
        self.stats_text.insert(tk.END, "\n\n=== BLOCK GENERATION RESULTS ===\n")
        self.stats_text.insert(tk.END, f"Generated {len(blocks)} blocks\n")
        self.stats_text.insert(tk.END, f"Residual rooms: {len(residuals)}\n\n")

        for i, block in enumerate(blocks[:20]):  # Show first 20 blocks
            self.stats_text.insert(tk.END, f"Block {i + 1}: {block}\n")

        if len(blocks) > 20:
            self.stats_text.insert(tk.END, f"\n... and {len(blocks) - 20} more blocks\n")

        # Update visualization
        self.show_blocks_var.set(True)
        self.update_output_display()

    def visualize_with_blocks(self):
        """Create visualization showing blocks"""
        if not self.floor_plan:
            return
        ...
        block_colors = {}
        for i, block in enumerate(self.floor_plan.blocks):
            block_colors[block.block_id] = plt.cm.tab20(i % 20)
            ...
            rect = Rectangle(
                ...
                block.height,
                linewidth=2,
                edgecolor=block_colors[block.block_id],
                facecolor='none',
                alpha=0.8
            )
            self.ax.add_patch(rect)
        ...
        for room in self.floor_plan.rooms:
            color = 'lightgray'
            if room.block_id and room.block_id in block_colors:
                color = block_colors[room.block_id]
        ...
        self._set_plot_limits()
        self.ax.set_title("Floor Plan with Blocks", fontsize=12)
